...
private:
	...
	std::vector<uint16_t> vids_pids_;
	int32_t timeout_wait_tuning_;
	int32_t timeout_wait_stream_;
	int32_t timeout_purge_stream_;
	int32_t wait_purge_stream_;
	int32_t required_purge_size_;

	usb_endpoint_st usb_endpoint_;
	it9175_state device_state_;
	tsthread_ptr ts_thread_;
